...
#+end_src
** Traefik Deployment
Follow the deployment instructions in the Traefik library module (Note that the
variable =<<SRC_DIR>>= points back to the directory containing /this current/
file, so files created from =traefik.org= will create file here in this
directory.):
...
#+end_src
** Flux Deployment
Follow the deployment instructions in the Flux library module:
