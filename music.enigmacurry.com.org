...
*** FUNKWHALE_DOMAIN
    ...
    <<CLUSTER>>
    #+end_src
*** FUNKWHALE_POSTGRESQL_CHART_VERSION
The [[https://github.com/bitnami/charts/blob/master/bitnami/postgresql/Chart.yaml][version number of the chart to install postgresql]]:
    #+name: FUNKWHALE_POSTGRESQL_CHART_VERSION
    #+begin_src config :noweb yes :eval no
    10.1.1
    #+end_src
*** FUNKWHALE_POSTGRESQL_PVC_SIZE
    Size of data volume for Funkwhale's postgresql:
    #+name: FUNKWHALE_POSTGRESQL_PVC_SIZE
    #+begin_src config :noweb yes :eval no
    10Gi
    #+end_src
*** FUNKWHALE_REDIS_CHART_VERSION
The [[https://github.com/bitnami/charts/blob/master/bitnami/postgresql/Chart.yaml][version number of the chart to install postgresql]]:
    #+name: FUNKWHALE_REDIS_CHART_VERSION
    #+begin_src config :noweb yes :eval no
    12.1.1
    #+end_src
*** FUNKWHALE_REDIS_PVC_SIZE
    #+name: FUNKWHALE_REDIS_PVC_SIZE
    #+begin_src config :noweb yes :eval no
    10Gi
    #+end_src
** Funkwhale Deployment
*** Add Bitnami Helm chart repository to Flux:
    #+begin_src shell :noweb yes :eval never-export :exports code :results none
...
*** Update Helm releases
    ...
    flux create helmrelease postgresql \
      ...
      --chart-version="<<FUNKWHALE_POSTGRESQL_CHART_VERSION>>" \
      --export > <<SRC_DIR>>/funkwhale/postgresql.release.yaml
    ...
    flux create helmrelease redis \
      ...
      --chart=redis \
      --target-namespace=funkwhale \
      --values=funkwhale/redis.values.yaml \
      --chart-version="<<FUNKWHALE_REDIS_CHART_VERSION>>" \
      --export > <<SRC_DIR>>/funkwhale/redis.release.yaml
    #+end_src
...
*** Check services:
    Wait a bit for Flux to deploy, then check the status of services:
    #+begin_src shell :noweb yes :eval never-export :exports code
    ...
    | pod/funkwhale-postgresql-postgresql-0            | 1/1       | Running      | 0           | 49m      |     |
    | NAME                                             | TYPE      | CLUSTER-IP   | EXTERNAL-IP | PORT(S)  | AGE |
    | service/funkwhale-postgresql-headless            | ClusterIP | None         | <none>      | 5432/TCP | 49m |
    | service/funkwhale-postgresql                     | ClusterIP | 10.43.19.196 | <none>      | 5432/TCP | 49m |
    | NAME                                             | READY     | AGE          |             |          |     |
    | statefulset.apps/funkwhale-postgresql-postgresql | 1/1       | 49m          |             |          |     |
    ...
    | data-funkwhale-postgresql-postgresql-0 | Bound  | pvc-41dcf17e-e41a-4ade-8974-4143a565a7c4 | 10Gi     | RWO    | local-path | 48m          |     |

    Check the helm controller log for important notes about how to access the
    database via =psql=:
    #+begin_src shell :noweb yes :eval never-export :exports code
    <<kubectl>> -n flux-system logs 
    #+end_src
...
*** funkwhale/postgresql.values.yaml
    ...
    #+end_src
*** funkwhale/redis.values.yaml
    #+begin_src yaml :noweb yes :eval no :tangle funkwhale/redis.values.yaml
    master:
      persistence:
        size: <<FUNKWHALE_REDIS_PVC_SIZE>>
    #+end_src
* LICENSE
