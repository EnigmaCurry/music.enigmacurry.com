...
directory. If you are ever unsure exactly what command is going to be run, you
can usually add an =echo= in front of the line so that the command is parsed but
not executed.
#+END_COMMENT
