...
kubectl create secret generic funkwhale \
   --namespace funkwhale --dry-run=client -o json \
   --from-literal=POSTGRES_USER=funkwhale \
   --from-literal=POSTGRES_PASSWORD=$(openssl rand --base64 24) \
  | kubeseal -o yaml > <<SRC_DIR>>/funkwhale/sealed-secret.yaml
#+end_src
...
*** TODO funkwhale/pvc.yaml
#+begin_src yaml :noweb yes :eval no :tangle funkwhale/pvc.yaml
apiVersion: v1
kind: PersistentVolumeClaim
metadata:
  name: funkwhale-postgres-data
  namespace: funkwhale
spec:
  accessModes:
  - ReadWriteOnce
...
---
#+end_src
*** TODO funkwhale/database.yaml
#+begin_src yaml :noweb yes :eval no :tangle funkwhale/database.yaml
apiVersion: v1
kind: Service
metadata:
...
spec:
  selector:
    matchLabels:
      app: funkwhale-postgres
  serviceName: funkwhale-postgres
  replicas: 1
  template:
    metadata:
      labels:
      ...
      volumes:
        - name: funkwhale-postgres-data
          persistentVolumeClaim:
            claimName: funkwhale-postgres-data
#+end_src
*** TODO funkwhale/kustomization.yaml
